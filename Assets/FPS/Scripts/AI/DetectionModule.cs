...
namespace Unity.FPS.AI
{
    public class DetectionModule : MonoBehaviour
    {
        ...
        [Tooltip("The max angle at which the enemy can see targets (FOV)")]
        public float DetectionAngleMax = 50f;

        [Tooltip("Toggle FOV angle calculation")]
        public bool IgnoreDetectionAngle = false;

        [Tooltip("The distance at which the enemy will detect target regardless of direction")]
        public float DetectionTooClose = 10f;
        ...
        public virtual void HandleTargetDetection(Actor actor, Collider[] selfColliders)
        {
            ...
            foreach (Actor otherActor in m_ActorsManager.Actors)
            {
                if (otherActor.Affiliation != actor.Affiliation)
                {
                    ...
                    {
                        float detectionAngle = Vector3.Angle(DetectionSourcePoint.forward, otherActor.transform.position);
                        //print("angle + " + detectionAngle);
                        if (detectionAngle > DetectionAngleMax && DetectionTooClose < sqrDistance && !IgnoreDetectionAngle && KnownDetectedTarget == null)
                            return;
                        ...
                        if (foundValidHit)
                        {
                            ...
                            if (hitActor == otherActor)
                            {
                                ...
                                foreach(Collider col in Physics.OverlapSphere(this.transform.position, 10f))
                                {
                                    EnemyController enemyController = col.gameObject.GetComponentInParent<EnemyController>();
                                    if(enemyController)
                                    {
                                        //print("Alerting others");
                                        enemyController.DetectionModule.TimeLastSeenTarget = Time.time;
                                        enemyController.DetectionModule.KnownDetectedTarget = otherActor.AimPoint.gameObject;
                                    }
                                }
