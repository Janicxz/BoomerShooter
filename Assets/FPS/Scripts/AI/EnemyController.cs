...
namespace Unity.FPS.AI
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        [Tooltip("Light for the bot vision/detection")]
        public Light BotLight;

        [Tooltip("Delete the character after death")]
        public bool RemoveOnDeath = true;

        [Tooltip("Animator for death animation")]
        public Animator animator;

        [Tooltip("Blood pool for death")]
        ...
        public void SetNavDestinationRandom(Vector3 destination, float roamRadius)
        {
            if (NavMeshAgent)
            {
                NavMeshHit navHit;
                NavMesh.SamplePosition(destination, out navHit, roamRadius, NavMesh.AllAreas);
               // NavMeshAgent.SamplePathPosition(NavMesh.AllAreas, 1f, out navHit);
                NavMeshAgent.SetDestination(navHit.position);
                //Debug.DrawLine(transform.position, navHit.position, Color.red, 5f);
                //print("Set destination " + navHit.position);
            }
        }
        ...
        public void UpdatePathDestination(bool inverseOrder = false)
        {
            if (IsPathValid())
            {
                ...
                {
                    /*if(PatrolPath.PathNodes[m_PathDestinationNodeIndex].name.Contains("Wait"))
                    {
        ...
        void OnDie()
        {
            ...
            if(animator != null)
            {
                animator.SetFloat("AnimationSpeedRandom", Random.Range(1f, 1.5f));
            ...
            if (!RemoveOnDeath)
            {
                ...
                // Disable collision so we can walk over corpses
